...
# DAHBOARD html-composition
app.layout = html.Div([
	
	html.Div([
		html.Img(
			src="assets/cryptoBIT.png",
			style={'width': '4.25%', 'float': 'left'}
			),
		html.H1(
			'. CryptoBit',
			style={'margin-bottom': '14',
				   'margin-top': '5', 'font-size': '35', 'float': 'left'}
			),
		html.Img(
			src="assets/prologo_UW.png",
			style={'width': '14%', 'float': 'right'}
			),
		html.Div(
			dcc.Markdown(marked_info),
			style={'float': 'right', 'margin-top': '23'}
			)
		],
		className='row'
	),
	...
	html.Div([
		html.Div([
			dcc.Markdown(marked_main),
			html.P('--------------- PAUSE ---------------', 
				   id='animation_info',
				   style={'margin-top': '34', 'font-size': '11'} 
				  ),
			html.Button('■', id='button_reset', n_clicks=0),
			html.Button('▶▮', id='button_play_stop', n_clicks=0)
			],
			style={'margin-top': '32', 'text-align': 'center'},
	...
	html.Div([
		...
		html.H2(
			...
			style={'margin-top': '16', 'font-size': '15', 'float': 'right'},
			className='three columns'
			),
		], 
		className='row'
